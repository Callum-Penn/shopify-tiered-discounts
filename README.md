# Tiered Discounts Shopify App

A Shopify app that applies tiered quantity-based discounts to products based on pricing tiers stored in product metafields.

## Overview

This app uses a **Shopify Function** (written in Rust) to automatically apply discounts to products in the cart based on quantity thresholds. The tiered pricing data is stored in a JSON metafield on each product, allowing flexible pricing per product.

### Features

- ✅ Automatic tiered quantity discounts
- ✅ Per-product pricing configuration via metafields
- ✅ Supports multiple products in cart
- ✅ Combines with other discounts
- ✅ Real-time discount calculation at checkout

## Tech Stack

- **Backend**: Node.js with Express
- **Frontend**: React with Vite
- **Function**: Rust (compiled to WebAssembly)
- **Discount Logic**: Shopify Functions API

## How It Works

1. **Product Configuration**: Each product has a `custom.pack_prices` metafield containing JSON with tiered pricing:
   ```json
   [
   ...
   ```
   Note: `unit_price` is in **pence** (383 = £3.83)

2. **Cart Processing**: When items are added to the cart, the Shopify Function:
   - Reads the metafield from each product
   - Finds the best applicable tier based on quantity
   - Calculates the discount amount
   - Applies the discount automatically

3. **Discount Application**: The discount applies to all eligible products in the cart and combines with other discounts.

## Key Files

- `extensions/tiered-quantity-discount/src/cart_lines_discounts_generate_run.rs` - Main discount logic
- `extensions/tiered-quantity-discount/src/cart_lines_discounts_generate_run.graphql` - GraphQL query for cart data
...
See [SETUP_AND_TEST.md](./SETUP_AND_TEST.md) for detailed setup instructions.
